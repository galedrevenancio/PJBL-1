...
while True:
    ...
    match opcao_central:
        ...
        case '4':
            ...
            if informacao_cadastro == 's':
                peso_user = float(input("Informe seu peso (kg): "))
                genero_user = input("Informe seu genero: m (Masculino) ou f (Feminino): ").lower()
                while genero_user not in ['m', 'f']:
                    print("Opção inválida! Tente novamente.")
                    genero_user = input("Informe seu sexo (m/f): ").lower()
            ...
            calorias = met * peso_user * tempo_horas * gasto_genero
        ...
        case '9':
            ...
            # hidratação
            if agua == 0:
                print("\nMeta de áqua não definida (use opção 2)")
            else:
